	...
	for (int j = 0; j < tbuf.size(); ++j){
		...
		for (int i = 0; i < atoi(results[1].c_str()); ++i){
			...
			for (int k = 0; k < tbuf[j].coltypes.size(); ++k){
				...
				string name = tbuf[j].colnames[k].substr(tbuf[j].colnames[k].length() - 3, tbuf[j].colnames[k].length());
				string c = " ";
				c[0] = '0' + inc;
				++inc;
				inc %= 10;
				...
				else if (def == "int" || def == "float" || def == "double" || def == "decimal"){
					query += c + ", ";
				}
...
bool migrate(vector<string> results, vector<string> &buffer, vector<Table>& tbuf){
	...
	for (int i = 0; i < tbuf[id].colnames.size(); ++i){
		if (find(fields_1.begin(), fields_1.end(), tbuf[id].colnames[i]) != fields_1.end()){
			...
			inserts1 += tbuf[id].colnames[i] + ", ";
		}
		else{
			arg.push_back(tbuf[id].colnames[i]);
			arg.push_back(tbuf[id].coltypes[i]);
			inserts2 += tbuf[id].colnames[i] + ", ";
		}
	}
	if (!add_new(arg2, buffer, tbuf))return false;
	if (!add_new(arg, buffer, tbuf))return false;
	inserts1 = inserts1.substr(0, inserts1.length() - 2);
	inserts2 = inserts2.substr(0, inserts2.length() - 2);
	string arg3 = "INSERT INTO " + results[2] + "(" + inserts1 + ")" + " SELECT " + inserts1 + " FROM " + results[1] + ";";
	string arg4 = "INSERT INTO " + results[3] + "(" + inserts2 + ")" + " SELECT " + inserts2 + " FROM " + results[1] + ";";
	buffer.push_back(arg3);
	buffer.push_back(arg4);
	buffer.push_back("DROP TABLE " + results[1] + ";");
	for (int i = 0; i < tbuf.size(); ++i){
		if (tbuf[i].name == results[1])tbuf[i].name = results[3];
		for (int a = 0; a < tbuf[i].colnames.size(); ++a){
			if (tbuf[i].colnames[a] == results[1] + "_id")tbuf[i].colnames[a] = results[3] + "_id";
		}
		for (int a = 0; a < tbuf[i].ones.size(); ++a){
			if (tbuf[i].ones[a] == results[1] + "_id")tbuf[i].ones[a] = results[3] + "_id";
		}
	}
...
/*
create article_47 url string price currency content longtext
create category date_created_on date name varchar
create user age int description longtext picture_url string
create tag description varchar second_priority float
relate oo category article_47
relate oo article_47 user
relate mm user tag
write creates.sql
insert 2
write inserts.sql
...
migrate article_47 article_47_part1 article_47_part2 content
write migrates.sql

select tag article_47_part2
write selects2.sql
*/
